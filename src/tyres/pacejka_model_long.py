...
SL_MAX = 0.15                     # [-] valid slip ratio window (unitless, not degrees)
PRESSURE_TARGET = 83.0           # [kPa]
PRESSURE_TOL = 5.0               # [kPa]
DECIMATE = 2                     # keep every Nth sample after filtering
...
def pacejka_long_force(p, kappa, Fz):
    """
...
def generate_coeffs(run_data, run_id, n_starts=8, seed=0):
    ...
    for fz in fz_bin_centers:
        sel = (np.abs(FZ - fz) < fz_bin_tol) & (np.abs(SL) > 0.1) & (np.abs(IA) < 0.6)
        if sel.sum() > 40:
    ...
    for fz in fz_bin_centers:
        ...
        if sel.sum() > 20:
            # least-squares slope through origin: slope = sum(SL*FX)/sum(SL^2)
    ...
    def residuals(free):
        p = expand(free)
        model = pacejka_long_force(p, SL, FZ)
        return (FX - model) / FZ
    ...
    rms_err = np.sqrt(np.mean((FX - pacejka_long_force(p_fit, SL, FZ)) ** 2))
    ...
    savemat(Path(f"../../data/coeffs/longitudinal/[{run_id}] hoosier_r20_tire_params_long.mat").expanduser().resolve(), {
        "coeffs": p_fit,
...
def plot_fx_vs_sl(run_data, run_id, fn_buckets):
    ...
    for k, fz in enumerate(fn_buckets):
        ...
        fx_pred = pacejka_long_force(p_fit, sl_vec, fz * np.ones_like(sl_vec))
    ...
    ax.grid(True)
    plt.tight_layout()
    fig.savefig(Path(f"../../figures/tyres/longitudinal/[{run_id}] Fx vs SL (long).png").expanduser().resolve(), dpi=150)
    plt.close(fig)


def plot_fx_vs_fn(run_data, run_id, sl_buckets):
    """
    Longitudinal force vs normal force, one curve per slip ratio in sl_buckets [-].
    ...
    for k, sl in enumerate(sl_buckets):
        fx_pred = np.abs(pacejka_long_force(p_fit, sl * np.ones_like(fz_vec), fz_vec))
    ...
    ax.grid(True)
    plt.tight_layout()
    fig.savefig(Path(f"../../figures/tyres/longitudinal/[{run_id}] Fx vs Fn (long).png").expanduser().resolve(), dpi=150)
    plt.close(fig)
